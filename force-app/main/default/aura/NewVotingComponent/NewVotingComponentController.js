({
    ...
    handleComponentEvent : function(component, event, helper) {
        ...
        if(isDataExists === false) {
            allEmployees.push({id:valueFromChild.id, Vote__c: 
valueFromChild.vote});
        }
    ...
    Submit : function(component, event, helper) {
        if(component.get('v.employeeIdWIthVote').length > 0) {
            ...
            action.setParams({
                allEmployees : 
JSON.stringify(component.get('v.employeeIdWIthVote'))
            });
            action.setCallback(this, function(response){
   ...
   openModel: function(component, event, helper) {
      ...
      var allEmployees =  component.get('v.employeeIdWIthVote');
       existingEmployee.forEach((eachExistingEmp) => {
           var result = allEmployees.filter(eachemp => eachExistingEmp.Id 
=== eachemp.id);
           if(result.length > 0) {
